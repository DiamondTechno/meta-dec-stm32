# Hook DEC external-DT into optee-os-stm32mp and (optionally) generate
# the optee conf.mk so the upstream recipe sees DEC DTSes as additional
# flavors.
...
ST_OPTEE_EXPORT_TA_REF_BOARD:stm32mpcommonmx = "${DEC_EXTDT_DTB}.dts"
ST_OPTEE_EXPORT_TA_OEMAKE_EXTRA:stm32mpcommonmx = "CFG_EXT_DTS=${STAGING_EXTDT_DIR}/${EXTDT_DIR_OPTEE}"

# ------------------------------------------------
# Generate optee conf for usage of EXTERNAL DT with DEC project devicetree
# ------------------------------------------------
autogenerate_conf_for_external_dt_dec() {
    ...
    for supported in ${SOC_OPTEE_CONFIG_SUPPORTED}; do
        ...
        for soc in ${STM32MP_SOC_NAME}; do
            ...
            echo "flavor_dts_file-${supported}-DEC = ${dtb_by_soc}" >> ${WORKDIR}/conf.external_dt
            if [ "${soc}" = "stm32mp13" ] || [ "${soc}" = "stm32mp15" ]; then
                if [ "${soc_package}" = "a" ] || [ "${soc_package}" = "d" ]; then
                    echo "flavorlist-no_cryp = \$(flavor_dts_file-${supported}-DEC)" >> ${WORKDIR}/conf.external_dt
                fi
            fi
            case ${CUBEMX_BOARD_DDR_SIZE} in
                512)
                    echo "flavorlist-512M = \$(flavor_dts_file-${supported}-DEC)" >> ${WORKDIR}/conf.external_dt
                    ;;
                1024)
                    echo "flavorlist-1G = \$(flavor_dts_file-${supported}-DEC)" >> ${WORKDIR}/conf.external_dt
                    ;;
            esac
            if ${@bb.utils.contains('MACHINE_FEATURES','m33td','true','false',d)}; then
                echo "flavorlist-M33-TDCID = \$(flavor_dts_file-${supported}-DEC)" >> ${WORKDIR}/conf.external_dt
            fi

            echo "flavorlist-${supported} += \$(flavor_dts_file-${supported}-DEC)" >> ${WORKDIR}/conf.external_dt
        done
        echo "" >> ${WORKDIR}/conf.external_dt
    done
...
python() {
    ...
    if "stm32mpcommonmx" in machine_overrides:
        d.appendVarFlag('do_configure', 'prefuncs', ' autogenerate_conf_for_external_dt_dec')
}
